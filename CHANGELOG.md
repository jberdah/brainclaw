...
- **Turn-owned exactly-once review is now the default** (pln#630). The autonomous `request_changes` **fix-cycle re-dispatch** (round 2+ of a review loop) now routes through the turn-owned attempt state machine instead of the legacy best-effort projection: each re-dispatched turn gets an immutable attempt record (`turn_id`/`run_id`/`nonce`) behind an atomic launch fence (reserve → arm → consume) so it is **spawned at most once** even under concurrent dispatch or crash-recovery, and `reconcileTurn` finalizes it from **read-strict, turn-keyed evidence** (the coordinator's completion sentinel from the ack-wrapper) rather than presence-based markers — closing the double-spawn and false-green windows the legacy path had. The fix-cycle itself (bump round + retain worktree + re-dispatch, cap → `blocked`) and a crash-strand self-heal (a bumped-but-never-launched round is detected and re-armed at a fresh epoch, bounded by the dispatch lease) run on this path. The **initial** reviewer dispatch (`bclaw_coordinate(review, open_loop)`) remains the legacy single-call projection for now (converting it is a follow-up). Shipped after four adversarially-reviewed PRs (finalization, fix-cycle, strand self-heal + §9 conformance harness, revoked-grant recovery), the exactly-once fence verified sound each round, plus a live end-to-end for both verdicts (real spawn → cmd.exe-written turn-keyed sentinel → harvest → `reconcileTurn` → close-on-approve / bump-on-request_changes). **Kill-switch:** `BRAINCLAW_TURN_OWNED_REVIEW=0` (also `false`/`off`/`no`) reverts to the legacy closer. Triply-safe: harvest only turn-owns a lane that (a) owns a reservation AND (b) has turn-keyed evidence — a reviewer that resolves to inbox/manual (no ack-wrapper, no sentinel) or any legacy-dispatched lane falls back to the legacy closer and still converges. No change to ideation/implementation loops or the MCP tool surface.
