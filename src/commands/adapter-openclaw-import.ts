...
export function runAdapterOpenclawImport(file: string | undefined, options: OpenclawImportOptions = {}): void {
  if (!memoryExists()) {
  ...
  if (options.dryRun) {
    const events = options.session
      ...
      : listRuntimeEventsFromFile(file as string);
...
function listRuntimeEventsFromFile(file: string): RuntimeEvent[] {
...
function printDryRunSummary(events: RuntimeEvent[], session?: string): void {
