...
interface ReconcileStoreResult {
  cwd: string;
  relative_path: string;
  project_name: string;
  project_mode: string;
  project_strategy: string;
  bootstrap_refreshed: boolean;
...
export async function runReconcile(options: ReconcileOptions = {}): Promise<void> {
  ...
  const stores: ReconcileStoreResult[] = [];
...
async function confirmBootstrapApply(yes?: boolean): Promise<void> {
  if (yes) {
    return;
  }
