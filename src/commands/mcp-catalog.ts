...
const MCP_WRITE_TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        intent: { type: 'string', enum: ['assign', 'consult', 'review', 'reroute', 'summarize', 'ideate'], description: 'Coordination intent. Only assign/review/reroute spawn worker processes; consult/ideate/summarize are inbox-only. "assign" creates a claim per target agent and spawns a worker on the brief. "consult" delivers the brief to the target inbox(es) WITHOUT creating claims and WITHOUT spawning — targets pick it up via their own bclaw_work. "review" creates a review candidate (and, with open_loop, a review loop). "ideate" opens an ideation loop with the task as the proposal seed and queues critic briefs to the inbox; it does NOT yet spawn critic agents (real critic spawning is pln#626 Phase 2). "reroute" releases the current claim and reassigns. "summarize" reads a thread and returns a summary.' },
        ...
        autoExecute: { type: 'boolean', description: 'Attempt to spawn target agents after delivery (default: true). Applies ONLY to the spawning intents assign/review/reroute; consult, ideate, and summarize are inbox-only and ignore autoExecute — setting it true on them is a no-op that returns a warning (pln#626). When false on a spawning intent, returns command_ready_manual with bash commands for the supervisor to run.' },
