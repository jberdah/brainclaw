  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        autoExecute: { type: 'boolean', description: 'Attempt to spawn target agents after delivery (default: true). Applies ONLY to the spawning intents assign/review/reroute. consult and ideate are inbox-only: they ignore autoExecute — setting it true is a no-op that returns a warning (pln#626); summarize just reads a thread and ignores it silently. When false on a spawning intent, returns command_ready_manual with bash commands for the supervisor to run.' },
