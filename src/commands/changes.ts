...
interface DiffEntry {
  id: string;
  entry_type: 'constraint' | 'decision' | 'trap' | 'handoff';
  section: string;
  text: string;
  created_at: string;
}
...
export function runDiff(options: DiffOptions = {}): void {
  ...
  const makeEntry = (section: string, type: DiffEntry['entry_type']) =>
    (e: { id: string; text: string; created_at: string }): DiffEntry => ({
      id: e.id, entry_type: type, section, text: e.text, created_at: e.created_at,
    });

  const stateEntries: DiffEntry[] = [
  ...
  const newPending: Candidate[] = listCandidates('pending').filter(c => c.created_at > sinceISO);
  const newAccepted: Candidate[] = listArchivedCandidates('accepted').filter(
    c => (c.resolved_at ?? c.created_at) > sinceISO,
  );
  ...
  if (options.json) {
    ...
    if (total === 0) {
      ...
      if (newPending.length > 0) {
        if (stateEntries.length > 0) console.log('');
