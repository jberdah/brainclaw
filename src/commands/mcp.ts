...
type ContextFormat = 'markdown' | 'json' | 'template';
    ...
    try {
      ...
      if (method === 'tools/call') {
        ...
        if (name === 'bclaw_read_handoff') {
          ...
          const handoff = state.open_handoffs.find(h => h.id === args.id);
        ...
        if (name === 'bclaw_get_agent_board') {
          ...
          for (const inst of board.resolved_instructions.slice(0, 10)) {
            lines.push(`- [${inst.id}] <${inst.layer}${inst.scope ? `:${inst.scope}` : ''}> ${inst.text}`);
          }
          sendResult(id, {
        ...
        if (name === 'bclaw_search') {
          ...
          const lines = results.map(r => `[${r.id}] (${r.section}) score=${r.score.toFixed(2)}: ${r.text.slice(0, 120)}`);
          sendResult(id, {
...
function normaliseFormat(value: unknown): ContextFormat {
