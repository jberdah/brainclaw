...
export function buildProjectDiscovery(options: BuildDiscoveryOptions = {}): ProjectDiscoveryProfile {
  ...
  let integrations: AgentIntegrationReadiness[] = [];
  try {
...
export function saveDiscoveryProfile(profile: ProjectDiscoveryProfile, cwd?: string): void {
...
export function loadDiscoveryProfile(cwd?: string): ProjectDiscoveryProfile | undefined {
  const filePath = path.join(memoryDir(cwd), 'discovery', DISCOVERY_PROFILE_FILE);
  if (!fs.existsSync(filePath)) return undefined;
  try {
    return JSON.parse(fs.readFileSync(filePath, 'utf-8')) as ProjectDiscoveryProfile;
  } catch {
    return undefined;
  }
}
...
export function renderDiscoverySummary(profile: ProjectDiscoveryProfile): string {
  const lines: string[] = [];
...
function discoverStaticFiles(cwd: string, files: readonly string[]): DiscoveredFile[] {
...
function discoverFiles(cwd: string, files: readonly string[], dirs: readonly string[]): DiscoveredFile[] {
  const results: DiscoveredFile[] = [];
  ...
  for (const relativeDir of dirs) {
    ...
    for (const entry of fs.readdirSync(dir).sort()) {
      if (!/\.(md|mdc)$/i.test(entry)) continue;
 ...
 */
export function isManagedByBrainclaw(filePath: string): boolean {
  try {
    const content = fs.readFileSync(filePath, 'utf-8').slice(0, 200);
    return content.includes('brainclaw') && (
      content.includes('Managed by brainclaw') ||
