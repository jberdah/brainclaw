...
export function listCapabilities(cwd?: string): ProjectCapability[] {
  const dir = resolveEntityDir('capabilities', cwd, 'read');
  if (!dir || !fs.existsSync(dir)) return [];
  const files = fs.readdirSync(dir).filter(f => f.endsWith('.json'));
  const items: ProjectCapability[] = [];
  for (const file of files) {
    try {
      const result = loadVersionedJsonFile<ProjectCapability>('capability', path.join(dir, file));
...
export function saveCapability(cap: ProjectCapability, cwd?: string): void {
...
export function deleteCapability(id: string, cwd?: string): boolean {
  const dir = resolveEntityDir('capabilities', cwd, 'read');
  if (!dir) return false;
...
export function listTools(cwd?: string): ProjectTool[] {
  const dir = resolveEntityDir('tools', cwd, 'read');
  if (!dir || !fs.existsSync(dir)) return [];
  const files = fs.readdirSync(dir).filter(f => f.endsWith('.json'));
  const items: ProjectTool[] = [];
  for (const file of files) {
    try {
      const result = loadVersionedJsonFile<ProjectTool>('tool', path.join(dir, file));
...
export function saveTool(tool: ProjectTool, cwd?: string): void {
  const dir = resolveEntityDir('tools', cwd, 'write');
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  saveVersionedJsonFile('tool', path.join(dir, `${tool.id}.json`), tool);
}

export function deleteTool(id: string, cwd?: string): boolean {
  const dir = resolveEntityDir('tools', cwd, 'read');
  if (!dir) return false;
  const fp = path.join(dir, `${id}.json`);
  if (!fs.existsSync(fp)) return false;
...
export function createTool(opts: {
  ...
  const tool: ProjectTool = {
    ...
    name: opts.name,
    description: opts.description,
    type: (opts.type ?? 'utility') as ProjectTool['type'],
