...
export function handleMcpReadToolCall(name, args = {}, context = {}) {
    ...
    if (name === 'bclaw_read_handoff') {
        ...
        if (handoff) {
            ...
            if (handoff.plan_id)
                text += `Plan: ${handoff.plan_id}\n`;
            if (handoff.contract) {
                const c = handoff.contract;
                text += '\n--- Contract ---\n';
                if (c.files_touched?.length)
                    text += `Files touched:\n${c.files_touched.map(f => `  - ${f}`).join('\n')}\n`;
                if (c.pre_conditions?.length)
                    text += `Pre-conditions:\n${c.pre_conditions.map(p => `  - ${p}`).join('\n')}\n`;
                if (c.post_conditions?.length)
                    text += `Post-conditions:\n${c.post_conditions.map(p => `  - ${p}`).join('\n')}\n`;
                if (c.tests_to_verify?.length)
                    text += `Tests to verify:\n${c.tests_to_verify.map(t => `  - ${t}`).join('\n')}\n`;
                if (c.linked_plans?.length)
