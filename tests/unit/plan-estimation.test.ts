...
import { parseEffortMinutes, buildEstimationReport } from '../../src/commands/estimation-report.js';
...
describe('buildEstimationReport', () => {
  ...
  it('stores estimated_effort on plan creation', () => {
    runPlan('task with estimate', { estimate: '30min' });
    ...
    assert.equal(plan!.estimated_effort, '30min');
  });
  ...
  it('stores actual_effort when provided', () => {
    runPlan('task with actual', { estimate: '1h' });
    const state = loadState(workspace.dir);
  ...
  it('computes ratio=3 when est=1h actual=20min', () => {
    const report = buildEstimationReport({ cwd: workspace.dir });
