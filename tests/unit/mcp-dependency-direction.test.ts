...
describe('MCP dependency direction (pln#622 PR1)', () => {
  ...
  it('(b) extracted boundary modules do not import commands/mcp.js', () => {
    for (const name of ['mcp-read-handlers.ts', 'mcp-contract.ts', 'mcp-presentation.ts', 'mcp-catalog.ts', 'mcp-write-support.ts', 'mcp-write-coordination.ts', 'mcp-write-claims.ts', 'mcp-write-sequences.ts', 'mcp-write-memory.ts', 'mcp-write-admin.ts', 'mcp-write-entities.ts']) {
